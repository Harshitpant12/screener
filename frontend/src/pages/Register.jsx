...
function Register() {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    const toastId = toast.loading('Initializing workspace via Google...');
    try {
      await googleAuth(credentialResponse.credential);
      toast.success('Workspace ready! Redirecting...', { id: toastId, duration: 2000 });
      setTimeout(() => {
        navigate('/dashboard'); // we go straight to dashboard, no need to login again!
      }, 1000);
    } catch (err) {
